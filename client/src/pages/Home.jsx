...
import { Shield, Scan, AlertTriangle, Users, Phone, Zap, Database, BarChart2, Clock } from 'lucide-react';
...
const Home = () => {
  ...
  const datasetStats = [
    { number: '549k+', label: 'Phishing Site URLs Samples', icon: <Database className="mx-auto mb-2 text-cyan-400" size={32} /> },
    { number: '2.2k+', label: 'India Spam SMS Samples', icon: <Users className="mx-auto mb-2 text-cyan-400" size={32} /> },
    { number: '5.5k+', label: 'SMS Spam Collection Samples', icon: <Users className="mx-auto mb-2 text-cyan-400" size={32} /> },
  ];

  const modelStats = [
    { number: '99.06%', label: 'Final Accuracy', icon: <BarChart2 className="mx-auto mb-2 text-blue-400" size={32} /> },
    { number: '0.9858', label: 'Final F1 Score', icon: <BarChart2 className="mx-auto mb-2 text-blue-400" size={32} /> },
    { number: '4 hrs', label: 'Training Duration (T4 GPU)', icon: <Clock className="mx-auto mb-2 text-sky-400" size={32} /> },
    { number: '2.0k+', label: 'Evaluation Samples', icon: <Database className="mx-auto mb-2 text-blue-400" size={32} /> },
  ];
  ...
  return (
    <div className="min-h-screen">
     ...
     <section className="py-16 bg-#19183B text-gray-800">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 mb-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
            {modelStats.map((stat, idx) => (
              ...
              >
                {stat.icon}
                <div className="text-4xl font-extrabold text-blue-300">{stat.number}</div>
      ...
      {/* Features Section */}
      <section className="py-20 bg-gradient-to-b from-[#03344f] to-[#065b7c]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((feature, index) => (
              <motion.div
                ...
                className="bg-[#04283e]/60 backdrop-blur-md p-8 rounded-xl border border-cyan-800 hover:shadow-[0_0_20px_#00bcd4] transition-all"
              >
                <div className={`inline-flex p-3 rounded-lg mb-4 bg-${feature.color}-100/20 text-${feature.color}-400`}>
                  {feature.icon}
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              ...
              className="bg-gradient-to-r from-[#00c6ff] to-[#0072ff] px-8 py-4 rounded-lg font-semibold hover:from-[#00e0ff] hover:to-[#00bcd4] transition-all duration-300"
            >
              Call: 1930-FRAUD-HELP
            </a>
            <Link
              to="/emergency"
              className="border-2 border-cyan-300 text-cyan-200 px-8 py-4 rounded-lg font-semibold hover:bg-cyan-300 hover:text-[#02283f] transition-colors"
            >
              Emergency Resources
            </Link>
          </div>
